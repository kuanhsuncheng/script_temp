...
function build_dbus_withsystemd()
{
    ...
    ./configure -q \
            ...
            LDFLAGS="$LDFLAGS_SETTING -L$TEMPDIR/lib -L$TOOLCHAIN_LIBC_LIB -L$TOOLCHAIN_LIBC_USR_LIB  -Wl,--rpath-link -Wl,$TEMPDIR/lib" \
            LIBS="-lffi -lz -lexpat -ldl -lresolv -lcap -lmount -lgcrypt -lgpg-error -lblkid -lkmod -llz4 -lrt -lm -lpthread -lsystemd" \
            SYSTEMD_CFLAGS="-I$TEMPDIR/usr/include" \
            ...
            --disable-static \
            --disable-selinux \
            --prefix="$ROOT_DIR/$DBUS_PACKAGE/install" || exit 1
